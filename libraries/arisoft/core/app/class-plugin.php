...
class Plugin {
	...
	public function init() {
        ...
        add_action(
            'wp_ajax_' . $ajax_postfix,
            function () {
				$this->ajax_dispatcher();
			}
        );
        add_action(
            'wp_ajax_nopriv_' . $ajax_postfix,
            function () {
				$this->ajax_dispatcher( true );
			}
        );
	}
	...
	public function __call( $name, $arguments ) {
        ...
        $override_params = isset( $arguments[1] ) && is_array( $arguments[1] ) ? $arguments[1] : null;

		switch ( $cmd_type ) {
			case 'display':
                $preset_action = null !== $override_params && ! empty( $override_params['action'] ) ? $override_params['action'] : null;
                $preset_page = null !== $override_params && ! empty( $override_params['page'] ) ? $override_params['page'] : null;

                $ctrl_name = $preset_action ? $preset_action : 'display';
                if ( $this->options->page_prefix ) {
                    $current_page = $preset_page ? $preset_page : Request::get_var( 'page' );
                    if ( strpos( $current_page, $this->options->page_prefix ) === 0 ) {
                        $ctrl_name = $preset_action ? $preset_action : Request::get_var( 'action', 'display' );
                    }
                } else {
                    $ctrl_name = $preset_action ? $preset_action : Request::get_var( 'action', 'display' );
                }
