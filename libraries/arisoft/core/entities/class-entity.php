...
class Entity {
    ...
    public function __construct( $tbl, $keys, $db, $with_prefix = true ) {
        if ( $with_prefix ) {
            $tbl = $db->prefix . $tbl;
        }
    ...
    public function get_unique_name( $name = null, $title_db_field = null ) {
        ...
        if ( ! in_array( $title_db_field, $db_fields ) ) {
            throw new \InvalidArgumentException( '"' . esc_html( $title_db_field ) . '" DB field does not exist.' );
        }
